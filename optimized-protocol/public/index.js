...
const militaryParameters = getElement('#militaryParameters');
const residenceParameters = getElement('#residenceParameters');
const criminalRecordParameters = getElement('#criminalRecordParameters');
const driverLicenseParameters = getElement('#driverLicenseParameters');
const taxDebtParameters = getElement('#taxDebtParameters');
const cityCode = getElement('#cityCode');
...
const driverLicenseInputs = driverLicenseParameters.querySelectorAll('input');
...
function updateProofParameters() {
  const military = documentType.value === 'military-service';
  const residence = documentType.value === 'residence';
  const criminalRecord = documentType.value === 'criminal-record';
  const driverLicense = documentType.value === 'driver-license';
  const taxDebt = documentType.value === 'tax-debt';
  proofParameters.hidden = !military && !residence && !criminalRecord && !driverLicense && !taxDebt;
  militaryParameters.hidden = !military;
  residenceParameters.hidden = !residence;
  criminalRecordParameters.hidden = !criminalRecord;
  driverLicenseParameters.hidden = !driverLicense;
  taxDebtParameters.hidden = !taxDebt;
  generationDate.disabled = !military;
  assertionMonths.disabled = !military;
  cityCode.disabled = !residence;
  for (const input of driverLicenseInputs) input.disabled = !driverLicense;
}
